...
export async function runScaffold(input) {
    ...
    // vite.config.ts
    write(p('vite.config.ts'), `import { defineConfig } from 'vite'
...
`);
    // tailwind.config.ts
    write(p('tailwind.config.ts'), `import type { Config } from 'tailwindcss'
...
`);
    // postcss.config.js
    write(p('postcss.config.js'), `export default {
  plugins: {
    tailwindcss: {},
    autoprefixer: {},
  },
...
`);
    // index.html
    write(p('index.html'), `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
`);
    // src/main.tsx
    write(p('src/main.tsx'), `import React from 'react'
...
`);
    // src/index.css
    write(p('src/index.css'), `@tailwind base;
@tailwind components;
@tailwind utilities;
...
`);
    // src/data/types.ts
    write(p('src/data/types.ts'), `export type SlideType =
  | 'hero'
  | 'section-divider'
  | 'stat-grid'
  | 'two-column'
...
`);
    // src/data/config.ts
    write(p('src/data/config.ts'), `import type { DeckConfig } from './types.ts'
...
`);
    // src/data/slides.ts
    write(p('src/data/slides.ts'), `import type { Slide } from './types.ts'
...
`);
    // src/lib/utils.ts
    write(p('src/lib/utils.ts'), `import { type ClassValue, clsx } from 'clsx'
...
`);
    ...
    write(p('src/hooks/useKeyboardNav.ts'), `import { useState, useEffect } from 'react'
...
`);
    ...
    write(p('src/components/slides/HeroSlide.tsx'), `import { motion } from 'framer-motion'
...
`);
    ...
    write(p('src/components/slides/SectionDividerSlide.tsx'), `import { motion } from 'framer-motion'
...
`);
    ...
    write(p('src/components/slides/StatGridSlide.tsx'), `import { motion } from 'framer-motion'
...
`);
    ...
    write(p('src/components/slides/CtaSlide.tsx'), `import { motion } from 'framer-motion'
...
`);
    ...
    write(p('src/components/chrome/SlideNav.tsx'), `interface SlideNavProps {
  current: number
  total: number
  onPrev: () => void
  onNext: () => void
...
`);
    ...
    write(p('src/components/chrome/ProgressBar.tsx'), `interface ProgressBarProps { current: number; total: number }
...
`);
    // src/App.tsx
    write(p('src/App.tsx'), `import { AnimatePresence, motion } from 'framer-motion'

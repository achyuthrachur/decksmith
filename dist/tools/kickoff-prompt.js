...
export const kickoffPromptSchema = z.object({
    mode: z.enum(['full', 'content_only', 'visual_only']).describe('Build mode: full (new deck from scratch), content_only (update slide content only), visual_only (redesign without content changes)'),
    agent: z.enum(['claude-code', 'codex', 'cursor', 'generic']).default('generic').describe('Target agent — returns the correct syntax and file references for this agent'),
});
export async function runKickoffPrompt(input) {
...
//# sourceMappingURL=kickoff-prompt.js.map
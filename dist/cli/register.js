...
function registerCodex(force) {
    ...
    if (!fs.existsSync(CODEX_CONFIG)) {
        ...
        if (existing.includes('[mcp_servers.decksmith]') && !force) {
            ...
            const updated = existing.replace(/\[mcp_servers\.decksmith\][^\[]*/, mcpBlock);
...
export function handleRegisterCommand(args) {

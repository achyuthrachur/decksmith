...
function globSrc(dir: string, ext: string): string[] {
...
export async function runQualityCheck(input: QualityCheckInput): Promise<string> {
  ...
  // Build report
  const categories = ['brand', 'content', 'technical', 'performance'] as const
  const lines: string[] = [`# Quality Check Report\nProject: ${projectPath}\n`]

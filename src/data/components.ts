...
export const reactbitsComponents: ComponentEntry[] = [
...
export const twentyFirstDevComponents: ComponentEntry[] = [
...
export function renderComponentTable(): string {
  const rb = reactbitsComponents.map(c =>
    `| ${c.name.padEnd(18)} | reactbits.dev | ${c.bestFor} |`
  ).join('\n')

  const tfd = twentyFirstDevComponents.map(c =>
    `| ${c.name.padEnd(18)} | 21st.dev      | ${c.bestFor} |\n|                    | Install: \`${c.installCmd}\` | |`
  ).join('\n')
